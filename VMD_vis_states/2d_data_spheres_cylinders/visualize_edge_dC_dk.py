...
# USAGE:
# ----------------------------------------
# python3 visualize_edge_dC_dk.py pdb_file [COM][ATOMIC] edge_metric_file output_vis_state_file_name output_vis_state_structural_file_name average_structure_file distance_cutoff value_cutoff
...
# ----------------------------------------
color_defs = {}
# colorid 32 is to be used to color nodes
color_defs[32] = "color change rgb 32 0.0 0.0 0.0\n"        # black
####    edges to be colored from purple to green
edge_high_centrality_rgb = np.array([0.25098039215686274, 0.0, 0.29411764705882354])                # #40004b; purple
edge_mid_centrality_rgb  = np.array([0.8862745098039215, 0.8862745098039215, 0.8862745098039215])   # #e2e2e2; gray
edge_low_centrality_rgb  = np.array([0.0, 0.26666666666666666, 0.10588235294117647])                # #00441b; green
...
for colorid in low_to_mid_colorids:
    ...
    thiscolor = np.abs(edge_low_centrality_rgb + index * (edge_mid_centrality_rgb - edge_low_centrality_rgb)/(len(low_to_mid_colorids)-1))
    ### VMD Stuff
    color_defs[colorid] = "color change rgb " + str(colorid) + " " + str(thiscolor[0]) + " " + str(thiscolor[1]) + " " + str(thiscolor[2]) + '\n'
    ### MATPLOTLIB Stuff
    cdict['red'].append((cmap_positions[colorid-edge_possible_colorids[0]],thiscolor[0],thiscolor[0]))
    cdict['green'].append((cmap_positions[colorid-edge_possible_colorids[0]],thiscolor[1],thiscolor[1]))
    cdict['blue'].append((cmap_positions[colorid-edge_possible_colorids[0]],thiscolor[2],thiscolor[2]))

for colorid in mid_to_high_colorids:
    index = colorid - mid_to_high_colorids[0]
    thiscolor = np.abs(edge_mid_centrality_rgb + index * (edge_high_centrality_rgb - edge_mid_centrality_rgb)/(len(mid_to_high_colorids)-1))
    ### VMD Stuff
    color_defs[colorid] = "color change rgb " + str(colorid) + " " + str(thiscolor[0]) + " " + str(thiscolor[1]) + " " + str(thiscolor[2]) + '\n'
    ### MATPLOTLIB Stuff
    cdict['red'].append((cmap_positions[colorid-edge_possible_colorids[0]],thiscolor[0],thiscolor[0]))
...
with open(edge_metric_file,'r') as W:
    for line in W:
        ...
        node1 = int(temp[0]) - 1 
        node2 = int(temp[1]) - 1 
        if temp[2] != 0. and distance_cutoff_matrix[node2,node1] == 1:
            edge_matrix[node1,node2] = edge_matrix[node2,node1] = float(temp[2])

edge_metric_min_max = (np.min(edge_matrix),np.max(edge_matrix))
edge_metric_abs_max = np.max(np.abs(edge_metric_min_max))
print(edge_metric_min_max, edge_metric_abs_max)
edge_metric_min_max = (-edge_metric_abs_max,edge_metric_abs_max)
...
for i in nNodes_range[:-1]:
    for j in nNodes_range[i+1:]:
        #if np.abs(edge_matrix[i,j]) > value_cutoff:     #
        if np.log10(np.abs(edge_matrix[i,j])) > value_threshold:     #data is kinda log normal... so using the log-normal distribution to apply the value threshold
...
for i in edge_radius_colorid_data:
    # radius
    i[-2] = ((2*np.abs(i[2]))/range_)*edge_radius_difference + edge_small_radius
...
#### sorting by colorid to prevent repetitive graphics calls...
...
cb.set_label(r'$\frac{d||C_{m,n}||^2}{dk_{i,j}}$ (10$^{-4}$ $\AA^{4}/($kcal mol$^{-1} \AA^{-2})$)',size=16)
